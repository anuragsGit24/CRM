...
$sql = 'SELECT id, name, contact, email, address, status FROM builders WHERE id = ? LIMIT 1';
$stmt = $conn->prepare($sql);
if (!$stmt) {
    echo 'Unable to load builder details right now.';
    exit;
}
...
<div class="layout">
    <header class="topbar">
        <div>
            <h1 class="title">Builder Details</h1>
            <p class="subtitle">Read-only builder profile for admin review.</p>
        </div>
        <a class="btn" href="<?php echo htmlspecialchars(app_url('/admin/builder/list_builder.php')); ?>">Back to Builders</a>
    </header>

    <section class="card">
        <div class="row"><div class="label">Name</div><div class="value"><?php echo htmlspecialchars($builder['name']); ?></div></div>
        <div class="row"><div class="label">Contact</div><div class="value"><?php echo htmlspecialchars($builder['contact']); ?></div></div>
        <div class="row"><div class="label">Email</div><div class="value"><?php echo htmlspecialchars($builder['email']); ?></div></div>
        <div class="row"><div class="label">Address</div><div class="value"><?php echo nl2br(htmlspecialchars($builder['address'])); ?></div></div>
        <div class="row"><div class="label">Status</div><div class="value"><span class="badge <?php echo ($builder['status'] ?? '') === 'active' ? 'active' : 'inactive'; ?>"><?php echo htmlspecialchars($builder['status']); ?></span></div></div>
    </section>
</div>
</body>
</html>
